...
const ChatContainer = ({selectedUser,setSelectedUser}) => {
  ...
  useEffect(()=>{
    if(scrollEnd.current){
      scrollEnd.current.scrollIntoView({behviour:"smooth"})
    }
  ...
  return selectedUser ? (
    ...
    <div className="h-full overflow-scroll  relative backdrop-blur-lg">
      ...
      <div className="flex items-center gap-3 py-3 mx-4 border-b border-stone-500">
        ...
        <img onClick={()=>setSelectedUser(null)} src={assets.arrow_icon} alt="icon" className="md:hidden max-w-7"/>
        <img src={assets.help_icon} alt="icon" className="max-md:hidden max-w-5" />
        ...
        {messagesDummyData.map((msg,index)=>(
          <div key={index} className={`flex items-end gap-2 justify-end
           ${msg.senderId !== '680f50e4f10f3cd28382ecf9' && 'flex-row-reverse'}`}>
            {msg.image ? (
              <img src={msg.image} alt="icon" className="max-w-[230px] border
              ...
              text-white ${msg.senderId === '680f50e4f10f3cd28382ecf9' ? 'rounded-br-none' : 'rounded-bl-none'}`}>
                {msg.text}
