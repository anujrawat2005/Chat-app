    ...
    text-white ${selectedUser ? "max-md:hidden":''}`}>
     <div className="pb-5 ">
        ...
        <div className="bg-[#282142] rounded-full  flex items-center gap-2 py-3 px-4 mt-5 ">
            ...
            <input type="text" className="bg-transparent border-none outline-none text-white text-xs" placeholder="search..User"/>
     ...
     <div className="flex flex-col">
        {userDummyData.map((user,index)=>(
            <div onClick={()=>{setSelectedUser(user)}} key={index} className={`relative flex items-center gap-2 p-2 pl-4 rounded cursor-pointer
            max-sm:text-sm${selectedUser?._id=== user._id && 'bg-[#282142]/50'}`}>
                <img src={user?.profilePic || assets.avatar_icon} alt="pics" className="w-[35px] aspect-[1/1] rounded-full" />
